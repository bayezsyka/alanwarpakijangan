<div class="max-w-7xl mx-auto px-3 sm:px-6 py-4 sm:py-8 lg:py-16">
    ...
    <div class="text-center mb-6 sm:mb-12 lg:mb-16">
        <div
            class="inline-flex items-center rounded-lg border-2 border-[#008362] px-4 sm:px-8 py-2 sm:py-4 transition-all duration-300 hover:bg-[#008362] hover:shadow-md group cursor-pointer mb-4 sm:mb-8">
            <span
                class="text-base sm:text-xl md:text-2xl font-bold tracking-wide text-[#008362] group-hover:text-white transition-colors duration-300">
                Artikel & Opini
            </span>
        </div>
        ...
        <div class="flex items-center justify-between gap-2 mb-4 sm:mb-8">
            ...
            <div
                class="flex items-center space-x-1 bg-gray-50 rounded-lg p-1 shadow-inner overflow-x-auto flex-1 min-w-0">
    ...
    <div class="flex-1">
        @if ($articles->isEmpty())
            <div
                class="bg-white rounded-xl sm:rounded-2xl shadow-sm p-6 sm:p-12 text-center max-w-md mx-auto border border-gray-100">
                <svg xmlns="http://www.w3.org/2000/svg"
                    class="h-10 w-10 sm:h-16 sm:w-16 mx-auto text-gray-300 mb-3 sm:mb-4" fill="none"
                ...
                <button wire:click="resetFilters"
                    class="px-4 sm:px-5 py-1.5 sm:py-2 bg-emerald-50 text-emerald-600 rounded-lg text-xs sm:text-sm font-medium hover:bg-emerald-100 transition-colors">
                    Reset Filter
                </button>
            </div>
        @else
            @if ($viewMode === 'grid')
                ...
                <div class="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6">
                    @foreach ($articles as $article)
                        <article
                            class="group bg-white rounded-xl sm:rounded-2xl overflow-hidden border border-gray-100 hover:shadow-xl transition-all duration-300 flex flex-col h-full">
                            <a href="{{ route('artikel.detail', $article->slug) }}" class="block h-full flex flex-col">
                                <div class="w-full aspect-[4/3] bg-gray-50 overflow-hidden relative">
                                    @php
                                        $imageUrl =
                                            $article->gambar &&
                                            Illuminate\Support\Str::startsWith($article->gambar, 'http')
                                                ? $article->gambar
                                                : ($article->gambar
                                                    ? asset('storage/' . $article->gambar)
                                                    : 'https://via.placeholder.com/800x600/f3f4f6/6b7280?text=Al-Anwar');
                                    @endphp
                                    ...
                                    @if ($article->category)
                                        <div class="absolute top-2 right-2 sm:top-4 sm:right-4">
                                            <span
                                                class="inline-block px-2 py-0.5 sm:px-3 sm:py-1 text-[10px] sm:text-xs font-semibold rounded-full bg-emerald-600 text-white">
                                ...
                                <div class="p-3 sm:p-6 flex-grow flex flex-col">
                                    <h3
                                        class="text-sm sm:text-xl font-bold text-gray-900 leading-snug mb-1 sm:mb-3 line-clamp-2">
                                        {{ $article->judul }}
                                    </h3>
            ...
            @else
                ...
                <div class="space-y-3 sm:space-y-4">
                    @foreach ($articles as $article)
                        <article
                            class="group bg-white rounded-xl sm:rounded-2xl overflow-hidden border border-gray-100 hover:shadow-xl transition-all duration-300">
                            <a href="{{ route('artikel.detail', $article->slug) }}" class="flex">
                                <div
                                    class="w-24 sm:w-1/4 aspect-square sm:aspect-[4/3] bg-gray-50 overflow-hidden relative flex-shrink-0">
                                    @php
                                        $imageUrl =
                                            $article->gambar &&
                                            Illuminate\Support\Str::startsWith($article->gambar, 'http')
                                ...
                                <div class="p-3 sm:p-6 flex-1 flex flex-col min-w-0">
                                    ...
                                    @endif
                                    <h3
                                        class="text-sm sm:text-xl font-bold text-gray-900 leading-snug mb-1 sm:mb-2 line-clamp-2">
                                        {{ $article->judul }}
                                    </h3>
                                    <p class="hidden sm:block text-gray-600 mb-4 line-clamp-2 text-sm">
