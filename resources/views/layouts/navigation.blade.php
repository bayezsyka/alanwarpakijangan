<nav x-data="{ open: false }" class="relative z-40">
    ...
    {{-- SIDEBAR UNTUK SELASANAN MANAGER --}}
    @if(Auth::check() && Auth::user()->isSelasananManager())
    <div class="hidden lg:flex lg:flex-col lg:fixed lg:inset-y-0 lg:left-0 lg:w-64
                bg-gradient-to-b from-emerald-800 via-emerald-700 to-emerald-600
                text-emerald-50 shadow-xl">
        ...
        {{-- Logo + brand --}}
        <div class="flex items-center justify-between h-16 px-4 border-b border-emerald-500/30">
            <a href="{{ route('manage.selasanan.index') }}" class="flex items-center gap-3">
                <img src="{{ asset('images/logo.webp') }}"
                     alt="Logo"
                     class="h-10 w-10 rounded-2xl bg-white/10 p-1.5">
                <div>
                    <p class="text-sm font-semibold tracking-wide">Pengurus Selasanan</p>
                    <p class="text-[11px] text-emerald-100/80">Panel Pengelolaan</p>
                </div>
            </a>
        </div>
            ...
            <div>
                ...
                <nav class="space-y-1">
                    <a href="{{ route('manage.selasanan.index') }}"
                       class="{{ request()->routeIs('manage.selasanan.*') ? $linkBase.' '.$linkActive : $linkBase.' '.$linkInactive }}">
                        <i class="fas fa-book-reader mr-3 text-emerald-200 group-hover:text-emerald-100"></i>
                        <span>Manajemen Selasanan</span>
                    </a>

                    <a href="{{ route('selasanan.index') }}" target="_blank"
                       class="{{ $linkBase.' '.$linkInactive }}">
                        <i class="fas fa-external-link-alt mr-3 text-emerald-200 group-hover:text-emerald-100"></i>
    ...
    {{-- TOP BAR + DRAWER MOBILE untuk Selasanan Manager --}}
    <div class="lg:hidden">
            ...
            <div class="flex items-center justify-between h-14 px-4 bg-white shadow-sm">
                <div class="flex items-center gap-2">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <p class="text-sm font-semibold text-emerald-800">Selasanan</p>
                            <p class="text-[11px] text-emerald-500">Pengurus</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="absolute inset-y-0 left-0 w-64
                    ...
                    <div class="flex items-center justify-between h-14 px-4 border-b border-emerald-500/30">
                        <div class="flex items-center gap-2 text-emerald-50">
                            ...
                            <div>
                                <p class="text-sm font-semibold">Pengurus Selasanan</p>
                    ...
                    {{-- Footer user + logout --}}
                    <div class="border-t border-emerald-500/30 px-4 py-3">
                        <div class="flex items-center justify-between gap-3 text-xs text-emerald-50">
                            <div>
                                <p class="font-semibold">{{ Auth::user()->name ?? 'Pengurus' }}</p>
                                <p class="text-emerald-100/80">{{ Auth::user()->email ?? '' }}</p>
                            </div>
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit"
